...
'              Password': [encrypted_pass]}
               # will save in same order (,) to csv file
...
def backup():
  ...
  cp = cp + "\PasswordManager_Backup\data.csv" # add FolderName & FileName to obtained path

  if not os.path.isdir('PasswordManager_Backup'): # If 'BackupPasswordManager' not exists
  
    os.makedirs('PasswordManager_Backup') # Create one, for back up
...
if not data_file: # if not then, create one
  ...
  print(textcolor.BOLD + "\n WELCOME TO PASSWORD MANAGER" + textcolor.ENDC)
